...
def test_absa_manifest_exists():
    ...
    assert Path("optional_tools/rpa/google_messages_absa/manifest.json").is_file()
...
def test_absa_scenario_exists():
    ...
    scenario_pack = json.loads(Path("optional_tools/rpa/google_messages_absa/scenario_pack.json").read_text(encoding="utf-8"))
    scenario = scenario_pack["scenarios"][0]
...
def test_absa_transaction_parser_extracts_2026_debit_order():
    from optional_tools.rpa.google_messages_absa.messages_tools import _parse_absa_transaction
...
async def test_absa_scroll_collection_includes_debug_metadata(monkeypatch):
    import optional_tools.rpa.google_messages_absa.messages_tools as messages_tools
