...
class BusinessDatasetSeedTests(unittest.TestCase):
    ...
    def test_seed_business_dataset_returns_record_counts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["record_counts"]["orders"], 8)
            self.assertEqual(result["record_counts"]["order_items"], 12)
            self.assertEqual(result["record_counts"]["shipments"], 5)
            self.assertEqual(result["record_counts"]["payments"], 8)
