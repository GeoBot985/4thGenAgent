...
from runtime.runtime_store import build_runtime_store_retention_plan, cleanup_runtime_store, ensure_runtime_store_layout
from runtime.taskframe import create_taskframe


def _seed_pending_runtime_store(runtime_root: Path) -> str:
...
def test_retention_plan_protects_pending_actions(tmp_path: Path) -> None:
    runtime_root = tmp_path / "runtime_data"
    frame_id = _seed_pending_runtime_store(runtime_root)

    plan = build_runtime_store_retention_plan(runtime_root)

    assert plan["dry_run"] is True
    assert plan["protected_count"] >= 1
    assert any(item["frame_id"] == frame_id for item in plan["protected"])


def test_cleanup_defaults_to_dry_run(tmp_path: Path) -> None:
    runtime_root = tmp_path / "runtime_data"
    _seed_pending_runtime_store(runtime_root)

    result = cleanup_runtime_store(runtime_root)
