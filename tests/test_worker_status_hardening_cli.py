...
def test_worker_status_json_includes_hardening(tmp_path, monkeypatch):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "src.taskframe_cli", "worker", "status", "--runtime-data-dir", str(runtime_data_dir), "--json"],
        cwd=str(repo),
        capture_output=True,
        text=True,
        check=False,
    ...
    assert {"ok", "classification", "anomalies", "recommendations"} <= set(payload["hardening"].keys())
