...
_SCENARIOS = [
    ...
    {
        "id": "supplier_invoice_match_happy_path",
        "label": "Supplier Invoice - Three-Way Match",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Matches supplier invoice to PO and receipt, stages match-run and ledger writes for approval.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4001", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 2, "executed_action_count": 0, "must_have_failure": False, "must_have_approval_pack": True, "required_outputs": ["invoice", "purchase_order", "receipts", "duplicate_check", "match_result", "exception_report", "match_run_write"], "required_pending_tools": ["sheet/write_rows", "supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_price_exception",
        "label": "Supplier Invoice - Price Exception",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Invoice unit price does not match the PO; exception report is generated and only the exception write is staged.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4002", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 1, "executed_action_count": 0, "must_have_failure": False, "must_have_approval_pack": True, "required_outputs": ["match_result", "exception_report", "match_run_write"], "forbidden_pending_tools": ["supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_quantity_exception",
        "label": "Supplier Invoice - Quantity Exception",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Invoice quantity exceeds received quantity; exception report is generated and only the exception write is staged.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4003", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 1, "executed_action_count": 0, "must_have_failure": False, "must_have_approval_pack": True, "required_outputs": ["match_result", "exception_report", "match_run_write"], "forbidden_pending_tools": ["supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_missing_receipt",
        "label": "Supplier Invoice - Missing Receipt",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Invoice has no receipt. Workflow generates an exception report and stages only the exception write.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4004", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 1, "executed_action_count": 0, "must_have_failure": False, "must_have_approval_pack": True, "required_outputs": ["match_result", "exception_report", "match_run_write"], "forbidden_pending_tools": ["supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_missing_po",
        "label": "Supplier Invoice - Missing PO",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Invoice references an unknown PO and the workflow fails safely.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4999", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"state_prefix": "FAILED", "pending_action_count": 0, "executed_action_count": 0, "must_have_failure": True},
    },
    {
        "id": "supplier_invoice_match_duplicate_invoice",
        "label": "Supplier Invoice - Duplicate Invoice",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Duplicate invoice number is detected and only the exception write is staged.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        "source": "operator_scenario_pack",
        "payload": {"invoice_ref": "SIN-4005", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 1, "executed_action_count": 0, "must_have_failure": False, "must_have_approval_pack": True, "required_outputs": ["duplicate_check", "match_result", "exception_report"], "forbidden_pending_tools": ["supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_approve_execute_dry_run",
        "label": "Supplier Invoice - Approve + Execute Dry Run",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Runs happy path, approves staged writes, and executes them in dry-run mode.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        ...
        "payload": {"invoice_ref": "SIN-4001", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "post_actions": [
            {"type": "approve_all_pending_actions", "approved_by": "scenario_pack", "reason": "Supplier invoice match approval dry-run."},
            {"type": "execute_approved_dry_run"},
        ],
        "expected": {"final_state": "COMPLETED", "pending_action_status": "EXECUTED", "pending_action_count": 2, "executed_action_count": 2, "must_have_failure": False, "must_have_approval_pack": True, "required_executed_tools": ["sheet/write_rows", "supplier_invoice/execute_ledger_write"]},
    },
    {
        "id": "supplier_invoice_match_report_generation",
        "label": "Supplier Invoice - Report Generation",
        "category": "accounting",
        "workflow_family": "supplier_invoice_matching",
        "scenario_type": "supplier_invoice_matching",
        "description": "Runs happy path and generates a report/evidence bundle while writes remain staged for approval.",
        "requires_dataset_seed": True,
        "uses_llm": True,
        "llm_mode": "fake_supplier_invoice_match",
        "event_type": "manual.supplier_invoice_match",
        ...
        "payload": {"invoice_ref": "SIN-4001", "tolerance_amount": 1.0, "tolerance_percent": 0.01},
        "generate_report": True,
        "expected": {"final_state": "WAITING_FOR_EXECUTE", "pending_action_count": 2, "executed_action_count": 0, "must_have_report": True, "must_have_evidence_bundle": True, "must_have_failure": False, "must_have_approval_pack": True},
    },
    {
