...
from runtime.business_store import get_customer as get_business_customer
from runtime.business_store import get_order as get_business_order
from runtime.business_store import get_order_items as get_business_order_items
from runtime.business_store import get_shipment as get_business_shipment
from runtime.business_store import get_payment as get_business_payment
from runtime.business_store import get_supplier as get_business_supplier
from runtime.business_store import get_supplier_invoice as get_business_supplier_invoice
from runtime.business_store import search_orders as search_business_orders
from runtime.business_store import search_inventory as search_business_inventory
from runtime.business_store import search_purchase_orders_open_by_sku as search_business_open_pos
from runtime.business_store import search_suppliers_active as search_business_active_suppliers
from runtime.business_store import load_business_dataset
...
def customer_read(customer_id: str) -> WorkspaceResult:
    customer = get_business_customer(customer_id, runtime_root="runtime_data")
    return WorkspaceResult(ok=customer is not None, action="customer_read", output=json.dumps(customer or {}, ensure_ascii=False), payload=customer)


def customer_search(customer_id: str = "", name: str = "", status: str = "") -> WorkspaceResult:
    customers = load_business_dataset("runtime_data")["customers"]
    results: list[dict[str, Any]] = []
    for customer in customers:
        if customer_id and str(customer.get("customer_id", "")).upper() != customer_id.strip().upper():
            continue
        if name and str(customer.get("name", "")).strip().lower() != name.strip().lower():
            continue
        if status and str(customer.get("status", "")).strip().lower() != status.strip().lower():
            continue
        results.append(dict(customer))
    return WorkspaceResult(ok=True, action="customer_search", output=json.dumps(results, ensure_ascii=False), payload=results)


def order_read(order_ref: str) -> WorkspaceResult:
    order = get_business_order(order_ref, runtime_root="runtime_data")
    return WorkspaceResult(ok=order is not None, action="order_read", output=json.dumps(order or {}, ensure_ascii=False), payload=order)


def order_search(customer_id: str = "", order_ref: str = "", status: str = "") -> WorkspaceResult:
    results = search_business_orders(customer_id=customer_id or None, order_ref=order_ref or None, status=status or None, runtime_root="runtime_data")
    return WorkspaceResult(ok=True, action="order_search", output=json.dumps(results, ensure_ascii=False), payload=results)


def order_items_list(order_ref: str) -> WorkspaceResult:
    items = get_business_order_items(order_ref, runtime_root="runtime_data")
    return WorkspaceResult(ok=True, action="order_items_list", output=json.dumps(items, ensure_ascii=False), payload=items)


def shipment_read(order_ref: str) -> WorkspaceResult:
    shipment = get_business_shipment(order_ref, runtime_root="runtime_data", prefer_order_ref=True)
    return WorkspaceResult(ok=shipment is not None, action="shipment_read", output=json.dumps(shipment or {}, ensure_ascii=False), payload=shipment)


def payment_read_by_order(order_ref: str) -> WorkspaceResult:
    payment = get_business_payment(order_ref, runtime_root="runtime_data")
    return WorkspaceResult(ok=payment is not None, action="payment_read_by_order", output=json.dumps(payment or {}, ensure_ascii=False), payload=payment)


def inventory_read(sku: str) -> WorkspaceResult:
    items = search_business_inventory(sku=sku, runtime_root="runtime_data")
    item = items[0] if items else None
    return WorkspaceResult(ok=item is not None, action="inventory_read", output=json.dumps(item or {}, ensure_ascii=False), payload=item)


def inventory_search_low_stock() -> WorkspaceResult:
    items = [item for item in search_business_inventory(runtime_root="runtime_data") if int(item.get("available_stock", 0) or 0) <= int(item.get("reorder_threshold", 0) or 0)]
    return WorkspaceResult(ok=True, action="inventory_search_low_stock", output=json.dumps(items, ensure_ascii=False), payload=items)


def supplier_read(supplier_id: str) -> WorkspaceResult:
    supplier = get_business_supplier(supplier_id, runtime_root="runtime_data")
    return WorkspaceResult(ok=supplier is not None, action="supplier_read", output=json.dumps(supplier or {}, ensure_ascii=False), payload=supplier)


def supplier_search_active() -> WorkspaceResult:
    suppliers = search_business_active_suppliers(runtime_root="runtime_data")
    return WorkspaceResult(ok=True, action="supplier_search_active", output=json.dumps(suppliers, ensure_ascii=False), payload=suppliers)


def purchase_order_search_open_by_sku(sku: str) -> WorkspaceResult:
    purchase_orders = search_business_open_pos(sku, runtime_root="runtime_data")
    return WorkspaceResult(ok=True, action="purchase_order_search_open_by_sku", output=json.dumps(purchase_orders, ensure_ascii=False), payload=purchase_orders)


def supplier_invoice_read(invoice_id: str) -> WorkspaceResult:
    invoice = get_business_supplier_invoice(invoice_id, runtime_root="runtime_data")
    return WorkspaceResult(ok=invoice is not None, action="supplier_invoice_read", output=json.dumps(invoice or {}, ensure_ascii=False), payload=invoice)


def order_context_build(customer: dict[str, Any], order: dict[str, Any], shipment: dict[str, Any]) -> WorkspaceResult:
    facts: list[str] = []
    customer_id = str(customer.get("customer_id", "")) if isinstance(customer, dict) else ""
    customer_name = str(customer.get("name", "")) if isinstance(customer, dict) else ""
    order_ref = str(order.get("order_ref", "")) if isinstance(order, dict) else ""
    shipment_id = str(shipment.get("shipment_id", "")) if isinstance(shipment, dict) else ""
    shipment_status = str(shipment.get("status", "")) if isinstance(shipment, dict) else ""
    estimated_delivery = str(shipment.get("estimated_delivery", "")) if isinstance(shipment, dict) else ""
    if customer_id and customer_name:
        facts.append(f"Customer {customer_id} is {customer_name}.")
    if order_ref and customer_id:
        facts.append(f"Order {order_ref} belongs to {customer_id}.")
    if order_ref and isinstance(order, dict) and order.get("status"):
        facts.append(f"Order {order_ref} status is {order.get('status')}.")
    if shipment_id and shipment_status:
        facts.append(f"Shipment {shipment_id} status is {shipment_status}.")
    if estimated_delivery:
        facts.append(f"Estimated delivery is {estimated_delivery}.")
    payload = {"customer": customer, "order": order, "shipment": shipment, "facts": facts}
    return WorkspaceResult(ok=True, action="order_context_build", output=json.dumps(payload, ensure_ascii=False), payload=payload)
