...
_CAPABILITIES: tuple[ToolCapability, ...] = (
    ...
    ToolCapability(
        tool_id="invoiceops",
        display_name="InvoiceOps",
        category="invoiceops",
        description="Fixture-backed invoice processing tools for reading, matching, reporting, and staged write planning.",
        core_or_optional="core",
        side_effect_level="prepare_only",
        auth_required=False,
        auth_type=None,
        setup_available=True,
        setup_action="Validate the local InvoiceOps fixtures and contract checks.",
        rpa_live_probe_required=False,
        limitations=["Uses local fixtures only and never performs live Google Sheets writes."],
    ),
    ToolCapability(
    ...
    for item in build_external_tool_capabilities():
        capabilities.append(
            ToolCapability(
                ...
                category=str(item.get("category", "toolpack")),
                description=str(item.get("description", "")),
                core_or_optional=str(item.get("core_or_optional", "optional")),
                side_effect_level=str(item.get("side_effect_level", "none")),
                auth_required=bool(item.get("auth_required", False)),
                auth_type=item.get("auth_type"),
                setup_available=bool(item.get("setup_available", False)),
                setup_action=item.get("setup_action"),
                rpa_live_probe_required=bool(item.get("rpa_live_probe_required", False)),
                excluded_from_default_release=bool(item.get("excluded_from_default_release", False)),
                source=str(item.get("source", "external_toolpack")),
                path=str(item.get("path", "")),
